...
void O3_CPU::operate()
{
  ...
  instrs_to_read_this_cycle = IFETCH_BUFFER.empty() && DISPATCH_BUFFER.empty() && DECODE_BUFFER.empty() && ROB.empty() ? 1 : 0;
  assert(IFETCH_BUFFER.occupancy() + DISPATCH_BUFFER.occupancy() + DECODE_BUFFER.occupancy() + ROB.occupancy() <= 1);
